...
Company facts and supplied imagery were taken from the MIMAG Company Profile provided with this project. The website intentionally avoids invented clients, testimonials, awards, certifications, statistics, social links, and project facts.

Some project cards use company-profile imagery as representative visuals. The UI explicitly notes that an image is not necessarily the photograph of the specific named project.
...
All website photos and image assets are stored under `assets/`. The company logo is `assets/logo.png`, general website imagery is under `assets/images/`, and registration/accreditation images are under `assets/images/certificates/`. The `favicon/` folder contains browser icon files. Keep new website images inside these folders and update the corresponding data path when needed.
